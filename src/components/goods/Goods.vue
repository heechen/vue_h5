<template>
  <div class="goods">
    <div class="menu-wrapper" ref="menuref">
      <ul>
        <li
          ...
          class="menu-item menu-list-hook"
    ...
    ></shopcart>
    <!-- food组件 -->
    <food :food=selectedFood ref="selectedFoodRef" @my-cartadd="myCartAdd"></food>
  </div>
</template>
...
export default {
  ...
  methods: {
    _drop (target) { // 在goods.vue定义 _drop方法将cartcontrol的传递过来target对象再传递给shopCart
      ...
      this.$refs.shopcartref.drop(target) // 父组件goods通过.$refs属性访问shopCart子组件的drop方法
      })
    },
    myCartAdd (target) { // 点击加号按钮触发事件
    ...
    selectMenu(index, event) {
      ...
      this.foodsScroll.scrollToElement(el, 300, 0, 0, "easing");
    },
    selectFood(food,event){
      ...
      this.$refs.selectedFoodRef.show()
    },
    empty(selectFoods){
